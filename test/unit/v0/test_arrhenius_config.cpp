...
TEST(ParserBase, CanParseValidArrheniusReaction)
{
  ...
  {
    auto parsed = parser.Parse(std::string("v0_unit_configs/reactions/arrhenius/valid") + extension);
    EXPECT_TRUE(parsed);
...
TEST(ParserBase, ArrheniusDetectsUnknownSpecies)
{
  ...
  for (auto& extension : extensions)
  {
    std::string file = std::string("v0_unit_configs/reactions/arrhenius/unknown_species") + extension;
...
TEST(ParserBase, ArrheniusDetectsMutuallyExclusiveOptions)
{
  ...
  for (auto& extension : extensions)
  {
    std::string file = std::string("v0_unit_configs/reactions/arrhenius/mutually_exclusive") + extension;
    auto parsed = parser.Parse(file);
    EXPECT_FALSE(parsed);
    EXPECT_EQ(parsed.errors.size(), 1);
...
TEST(ParserBase, ArrheniusDetectsBadReactionComponent)
{
  ...
  for (auto& extension : extensions)
  {
    std::string file = std::string("v0_unit_configs/reactions/arrhenius/bad_reaction_component") + extension;
    auto parsed = parser.Parse(file);
    EXPECT_FALSE(parsed);
...
TEST(ParserBase, ArrheniusDetectsUnknownPhase)
{
  ...
  for (auto& extension : extensions)
  {
    std::string file = std::string("v0_unit_configs/reactions/arrhenius/missing_phase") + extension;
    auto parsed = parser.Parse(file);
    EXPECT_FALSE(parsed);
    EXPECT_EQ(parsed.errors.size(), 1);

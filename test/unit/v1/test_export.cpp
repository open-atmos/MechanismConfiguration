...
#include <mechanism_configuration/v1/export.hpp>
...
TEST_F(ExportFixture, CanExportSpecies) {
    v1::types::Mechanism mechanism;
    v1::Export exporter;

    mechanism.species.push_back(v1::types::Species());
    mechanism.species[0].name = "H";
    mechanism.species.push_back(v1::types::Species());
    mechanism.species[1].name = "O";
    mechanism.species[1].optional_numerical_properties["molecular_weight"] = 16.0;
    ...
    v1::types::Mechanism yaml_mechanism = *parsed_yaml;
    ...
    EXPECT_EQ(yaml_mechanism.species[1].optional_numerical_properties.size(), 1);
    EXPECT_EQ(yaml_mechanism.species[1].optional_numerical_properties["molecular_weight"], 16.0);

    auto parsed_json = parser.Parse(file_json);
    EXPECT_TRUE(parsed_json);
    v1::types::Mechanism json_mechanism = *parsed_json;
    ...
    EXPECT_EQ(json_mechanism.species[1].optional_numerical_properties.size(), 1);
    EXPECT_EQ(json_mechanism.species[1].optional_numerical_properties["molecular_weight"], 16.0);
}

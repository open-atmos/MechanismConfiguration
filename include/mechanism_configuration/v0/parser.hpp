...
namespace mechanism_configuration
{
  ...
  class ConfigReaderPolicy
    {
     ...
     private:
      ...
      void ValidateSchema(const objectType& object, const std::vector<std::string>& required_keys, const std::vector<std::string>& optional_keys)
      {
        YAML::Emitter out;
  ...
  class ParserV0 : public Parser
  {
   public:
    /// @brief Determines if a YAML node can be parsed
    /// @param node a YAML node
    /// @return True if the node can be parsed, false otherwise
    bool CanParse(const YAML::Node& node) override;

    /// @brief Determines if a file path can be parsed
    /// @param file_path a path to a file
    /// @return True if the file can be parsed, false otherwise
    bool CanParse(const std::filesystem::path& file_path) override;
    ...
    std::pair<ConfigParseStatus, v0::Mechanism> Parse(const YAML::Node& node) override;

    /// @brief Reads a configuration from a file path
    /// @param file_path A path to single YAML configuration
    /// @return A pair containing the parsing status and mechanism
    std::pair<ConfigParseStatus, v0::Mechanism> Parse(const std::filesystem::path& file_path) override;

    /// @brief Reads a configuration from a file path
    /// @param file_path A path to single YAML configuration
    /// @return A pair containing the parsing status and mechanism
    std::pair<ConfigParseStatus, v0::Mechanism> Parse(const std::string& file_path) override;
  };
  }  // namespace v0
}  // namespace mechanism_configuration

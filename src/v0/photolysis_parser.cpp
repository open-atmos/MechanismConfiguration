  ...
  namespace v0
  {
    ConfigParseStatus PhotolysisParser(std::unique_ptr<types::Mechanism>& mechanism, const YAML::Node& object)
    {
      ...
      std::vector<std::function<void()>> parseSteps = {
        [&](){ if (status == ConfigParseStatus::Success) status = ParseReactants(object[validation::REACTANTS], reactants); },
        [&](){ if (status == ConfigParseStatus::Success) status = ParseProducts(object[validation::PRODUCTS], products); }
      };
